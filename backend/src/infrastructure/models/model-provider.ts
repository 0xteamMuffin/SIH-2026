...
export async function askModel(profile: ModelProfile, system: string, prompt: string) {
  ...
  // The AI SDK provider is the compatibility boundary. Requests use the common
  // OpenAI chat-completions wire format accepted by OpenRouter and local servers.
  void createOpenAICompatible(settings);
