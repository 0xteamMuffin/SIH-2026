...
Here's the diagram. The one decision that matters most: the API doesn't do the actual work. It just writes the job to Postgres, drops it in a queue, and a separate worker picks it up and runs it. Why does that matter? Because it means a run can survive a crash, get resumed, or get cancelled — it's not just some HTTP request that dies the second the connection drops. The browser only ever talks to the API, and the API checks who you are, your role, and your workspace before letting anything run. Every document goes through OCR and layout extraction first — so whether it's a clean PDF or a messy scanned report, we end up with structured text we can actually cite back to a page, not just a wall of text. Postgres holds the structured stuff — users, roles, workspaces, runs, evidence, audit logs. MinIO holds the actual files. Qdrant holds the search index, but we treat that as disposable — if it ever breaks, we just rebuild it from Postgres, so nothing real is lost. And which model handles what is just a config file, not code — so swapping a model out later is a config change, not a rewrite. That's the whole "no vendor lock-in" thing.

**Sovereignty / sandbox (13:30)**

Two words we're careful with here: sovereign and air-gapped. Sovereign just means the organization owns everything — the data, the storage, the model, who can access it, the logs. Air-gapped is the bigger claim: no route to the internet at all, period. And that has to actually be true at the network level — not just "we told the model not to." Concretely: sending anything to a remote model requires four things to line up at once — it has to be turned on, the provider has to be flagged remote, there has to be a valid key, and the data itself has to be marked public or synthetic. Miss any one of those and nothing gets sent. And anything marked internal or confidential just can't go remote, ever, no matter what else is switched on. In the sovereign setup, we don't even leave that path lying around — there's no internet connection for the backend to use even if it wanted to. Same idea for code: it never runs inside the API. It goes to a separate sandbox, and only after a human reviewer looks at the actual code and approves it — disposable container, no internet, read-only, hard limits on resources. The API just gets back the output and whether it worked. One honest caveat: this removes our own way out, but a real deployment still needs the org's actual firewall backing it up. We're not the whole story there.

**Status + roadmap (14:30)**

What's actually working right now: role-based access with reviewer sign-off, runs that survive a restart, OCR on documents, vision on scanned pages, search with access control, generating real DOCX/PPTX/XLSX files, approval-gated code execution, and a fully audited API. What's next: running the search/embedding model fully locally so we don't need any outside call at all, better search ranking, and a live view so a reviewer can watch what's happening while a run is still going instead of just seeing the end result.

**Close (15:00)**

Bottom line: this takes confidential document and code work and actually hands it to an AI safely — a human still signs off on anything that matters, and none of it ever leaves the building. Thanks.
